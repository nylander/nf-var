...
"""
Helper script 02_cons_stats_INDIV.py for nf-var
"""
...
parser.add_argument(
        "-c", "--cons_fn_list",
        help="List of consensus files --> paths",
        action="store", nargs='+', default=[], required=True)

parser.add_argument(
        "-i", "--individual",
        help="Individual to which consensus sequences belong",
        required=True)

parser.add_argument(
        "-o", "--out_dir",
        help="Directory to store the tsv file, if unspecified assumes current work dir",
...
for consensus in consensus_list:
    ...
    cons_df = pd.DataFrame([[chromo, cons_len, cons_n, cons_n_rel]],
                           columns=['Chromo', 'Length', 'Missing_data', 'Missing_data_PERCENTAGE'])
...
total_df = pd.DataFrame([['Total', total_len, total_n, total_n_rel]],
                        columns=['Chromo', 'Length', 'Missing_data', 'Missing_data_PERCENTAGE'])
