...
"""
Helper script 03_cons_stats_SUMMARY.py for nf-var
"""
...
parser.add_argument(
        ...
        help="List of consensus stats files --> paths",
        action="store", nargs='+', default=[], required=True)

parser.add_argument(
        "-o", "--out_dir",
        help="Directory to store the overview file, if unspecified assumes current work dir",
...
for indiv in consensus_stats_list:
    ...
    indiv_sum_df = pd.DataFrame([[indiv_id, indiv_df.loc['Total']['Missing_data'],
                                  indiv_df.loc['Total']['Missing_data_PERCENTAGE']]],
                                columns=['Indiv', 'Missing_data', 'Missing_data_PERCENTAGE'])
