...
"""
Helper script 01_cov_stats_SUMMARY.py for nf-var
"""
...
parser.add_argument(
        "-c", "--indiv_cov_tsv_list",
        help="Samtools coverage summary per individual, fn list",
        action="store", nargs='+', default=[], required=True)

parser.add_argument(
        "-s", "--sex_chr_list",
        help="List of sex chromosomes if any",
        action="store", nargs='+', default=[], required=False)

parser.add_argument(
        "-p", "--plots_per_panel",
        help="How many indivs to plot next to each other?",
...
df = pd.DataFrame(columns=['#rname', 'startpos', 'endpos', 'numreads',
...
def find_chromo_type(chrom):
    """
    Set chromosome type
    """
    if chrom in sex_chr_list_updated:
